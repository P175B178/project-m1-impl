...
void WardenApp::run(std::stop_token appStopToken) { // NOLINT(performance-unnecessary-value-param)
  led.setMode(LedColor::Green, false);

  auto sensorThreadFn = [this](std::stop_token threadStopToken) { sensorLoop(std::move(threadStopToken)); };
  std::jthread sensorThread{sensorThreadFn};

  // When the app requests stop, propagate it to the sensor thread and
  // the queue so the control loop unblocks and exits cleanly.
  std::stop_callback stopOnRequest{appStopToken, [&] {
