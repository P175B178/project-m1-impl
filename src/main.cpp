...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "c:vh", longOpts, nullptr)) != -1) {
        switch (opt) {
            ...
            case 'v': printVersion(); return EXIT_SUCCESS;
            case 'h': printHelp(argv[0]); return EXIT_SUCCESS;
            default:  printHelp(argv[0]); return EXIT_FAILURE;
        }
    }
    ...
    spdlog::info("Temp threshold: {:.1f}°C, humidity threshold: {:.1f}%",
                 config.temperatureThreshold, config.humidityThreshold);
    spdlog::info("Read interval: {}s, averaging window: {} samples",
                 config.readInterval.count(), config.averagingWindow);
