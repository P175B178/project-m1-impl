...
Dht22Sensor::Dht22Sensor(std::string sysfsBasePath,
                         ...
                         float minHumidity,
                         float maxHumidity)
    : sysfsBasePath_{std::move(sysfsBasePath)}
    , minTemperature_{minTemperature}
    , maxTemperature_{maxTemperature}
    , minHumidity_{minHumidity}
    , maxHumidity_{maxHumidity} {}
...
std::expected<warden::Reading, warden::SensorError> Dht22Sensor::read() const {
    ...
    if (!hum) return std::unexpected{hum.error()};
...
std::expected<float, warden::SensorError>
Dht22Sensor::readSysfsValue(const std::string& path, float scale) const {
    std::ifstream file{path};
    if (!file.is_open()) return std::unexpected{warden::SensorError::ReadFailure};

    int raw{};
    if (!(file >> raw)) return std::unexpected{warden::SensorError::ReadFailure};
