...
// sleep_for has ~100µs jitter on Linux, so keep the period well above that.
constexpr int          toneFreqHz = 2000;
constexpr auto         tonePeriod = std::chrono::microseconds{1'000'000 / toneFreqHz};
// GPIO 17, physical pin 11 — fixed by hardware wiring (see Appendix A).
