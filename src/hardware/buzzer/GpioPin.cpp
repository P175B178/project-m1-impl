...
struct LineConfigDeleter   { void operator()(gpiod_line_config*   p) const noexcept { gpiod_line_config_free(p);   } };
struct LineSettingsDeleter { void operator()(gpiod_line_settings* p) const noexcept { gpiod_line_settings_free(p); } };
struct RequestConfigDeleter{ void operator()(gpiod_request_config* p) const noexcept { gpiod_request_config_free(p); } };
...
GpioPin::GpioPin(const std::string& chipPath, unsigned int lineOffset, const std::string& consumer)
    ...
    if (!chip_) throw std::runtime_error{"Cannot open GPIO chip: " + chipPath};

    auto lineSettings = std::unique_ptr<gpiod_line_settings, LineSettingsDeleter>{gpiod_line_settings_new()};
    if (!lineSettings) throw std::runtime_error{"Cannot allocate line settings"};
    ...
    if (!lineConfig) throw std::runtime_error{"Cannot allocate line config"};
    ...
    if (!reqConfig) throw std::runtime_error{"Cannot allocate request config"};
    ...
    if (!request_) throw std::runtime_error{"Cannot request GPIO line " + std::to_string(lineOffset)};
}
...
void GpioPin::setHigh() {
    gpiod_line_request_set_value(request_, offset_, GPIOD_LINE_VALUE_ACTIVE);
}
